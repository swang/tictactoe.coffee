...
(function() {
  ...
  root.tictactoe.ai = (function($) {
    ...
    winnerWhere = function(boardState, player) {
      var c, col, csub, dc1, diag1, diag2, dr1, dsub, iters, r, _i, _j, _k, _ref, _ref1, _ref2;
      if (boardState == null) {
        boardState = board;
      }
      if (!(player != null)) {
      ...
      for (dc1 = _i = 0, _ref = config.rows - config.numToWin + 1 - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; dc1 = 0 <= _ref ? ++_i : --_i) {
        for (dr1 = _j = 0, _ref1 = config.cols - config.numToWin + 1 - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; dr1 = 0 <= _ref1 ? ++_j : --_j) {
          diag1 = [];
          diag2 = [];
          for (dsub = _k = 0, _ref2 = config.numToWin - 1; 0 <= _ref2 ? _k <= _ref2 : _k >= _ref2; dsub = 0 <= _ref2 ? ++_k : --_k) {
